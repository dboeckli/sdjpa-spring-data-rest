...
public class BeerWebController {
    ...
    @PostMapping("/" + BEER_TEMPLATE + "/save-new-beer/")
    ...
    public String searchBeers(Model model, @RequestParam(required = false) String beerName,
            @RequestParam(required = false) BeerStyleEnum beerStyle, @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "25") int size) {
        Page<Beer> beerPage;
